...
const renderDots = (colors, target) => {
  if (!target) return;
...
renderDots(pinkColors, document.getElementById("pink-dots"));
renderDots(peachColors, document.getElementById("peach-dots"));
...
if (installTarget) {
  installTarget.innerHTML = installGuides
    .map(
      (item) => `
...
const resourceTarget = document.getElementById("resource-grid");
if (resourceTarget) {
  resourceTarget.innerHTML = resources
    .map(
      (item) => `
